...
import pine.toast.toastrpg.core.events.WorldEventAlertEvent
import pine.toast.toastrpg.core.events.WorldEventAlertNowEvent
...
class WorldEventManager {
    ...
    fun areWeThereYet(destination: WorldEvent) {
        ...
        if(worldEventTime.check()){
            ToastRPG.getPassedPlugin()!!.server.pluginManager.callEvent(
                WorldEventAlertEvent(worldEvent,
                    WorldEventStage.getStage(worldEventTime.getLastCheck())
                )
            )
        }
    }
